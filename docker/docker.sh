...
set +e
bash -e <<TRY
  docker build -f ${PARENT_DIR}/docker/Dockerfile -t axiom:latest ./
TRY
if [ $? -ne 0 ]; then
  printf "\n*** Detected error running 'docker build'. Trying 'docker buildx' instead...\n"
  docker buildx build -f {PARENT_DIR}/docker/Dockerfile -t axiom:latest ./
fi

DOCKERIP=`docker network inspect bridge --format='{{(index .IPAM.Config 0).Gateway}}'`
docker run -it -d \
  --add-host=docker-host:$DOCKERIP \
  --env-file "${PARENT_DIR}/.env" \
  --hostname axiom \
  --name axiom \
  --volume ${PARENT_DIR}:/root/axiom:rw \
  axiom:latest
if [ $? -ne 0 ]; then
  kill "$PPID"; exit 1;
fi
